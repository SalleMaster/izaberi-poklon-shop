...
// async function updateCartData(userId: string): Promise<void> {
...
//   // Calculate total prices based on remaining items
//   const allItemsPrice = cart.items.reduce((acc, item) => acc + item.price, 0)
//   const deliveryFee =
//     cart.items.length > 0
//       ? cart.items.sort((a, b) => b.deliveryFee - a.deliveryFee)[0].deliveryFee
//       : 0

//   // Free shipping threshold (copied from your actions.ts)
//   const freeShippingThreshold = 5000 // Adjust this value to match your constant
...
//   // Handle coupon logic if present
//   if (cart.coupon) {
...
//       !isCouponExpired && isCouponActive && isCouponAvailable && isMinCartValue

//     if (!couponConditions) {
//       // Disconnect invalid coupon
...
//       } else if (cart.coupon.discountType === 'percentage') {
//         discount = Math.floor((allItemsPrice * cart.coupon.discount) / 100)
